...
class OCRGoogleVisionAPI(OCRBase):
    ...
    def send_to_google_vision(self, image_buffer: bytes):
        ...
        response = requests.post(VISION_API_URL, headers=headers, json=request_body, proxies=proxies)

        if response.status_code != 200:
            raise Exception(f"Error during request to Google Vision API: {response.text}")
